...
const { isActive, error, startCamera, stopCamera, captureFrame, switchCamera } = useCamera()
...
<template>
  ...
  >
    <div
      class="relative w-full h-full sm:h-auto sm:max-w-2xl sm:mx-4 sm:rounded-4xl overflow-hidden bg-black shadow-2xl flex flex-col"
    >
      <!-- Header Overlay -->
      <div
      ...
      <div class="relative flex-1 flex items-center justify-center bg-black overflow-hidden">
        ...
        <div v-if="error" class="absolute inset-0 flex items-center justify-center p-8">
          <div class="apple-glass p-6 rounded-3xl bg-red-500/10 border-red-500/20 text-center">
      ...
      <!-- Shutter Controls -->
      <div
        class="absolute bottom-0 left-0 right-0 p-8 flex items-center justify-center bg-linear-to-t from-black/80 to-transparent"
      >
        ...
        >
          ...
          >
            <path d="m4 16 4-4" />
            <path d="M20 8v12a2 2 0 0 1-2 2h-2" />
            <path d="m20 20-4-4" />
            <path d="M4 16v-6" />
            <path
              d="M4 8v-.667A2.667 2.667 0 0 1 6.667 4.667h10.666A2.667 2.667 0 0 1 20 7.333V8"
            />
          </svg>
        </button>
      </div>
